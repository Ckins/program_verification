...
# replace heap label in seq type
def replace_heap_in_seq(axioms1, axioms2, label):
    if (label == "*"): # temporary label, generated by this system
        for i in range(len(axioms1)):
            axioms1[i] = axioms1[i].replace("heap'(", "heap*(")
        for i in range(len(axioms2)):
            axioms2[i] = axioms2[i].replace("heap(", "heap*(")
    else:
        for i in range(len(axioms2)):
            axioms2[i] = axioms2[i].replace("heap(", "heap"+label+"(")
...
def seq_type(program, variables):
    ...
    if label_num=='-1': #the first statement has no label
        ...
        # replace heap
        replace_heap_in_seq(axioms1, axioms2, "*")

    else: #the first statement has a label
        replace_vars_in_axiom(axioms2, '', LABEL+label_num, variables)
